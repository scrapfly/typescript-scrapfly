...
export class ScrapeConfig {
  ...
  constructor(options: {
    ...
    this.headers = options.headers ? Object.fromEntries(Object.entries(options.headers).map(([k, v]) => [k.toLowerCase(), v])) : {};
  ...
  toApiParams(options: { key: string }): Record<string, any> {
    ...
    if (Object.keys(this.cookies).length > 0) {
      const cookiesAsHeader = [...Object.entries(this.cookies)].map(([key, value]) => `${key}=${value}`).join('; ');
